...
export default function WaterDemandModal({ data }) {
  const handleDownload = () => {
    ...
    doc.text("Plumbing - RWH Report", 10, 23);

    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(16);
    doc.text("Discharge:", 10, 40);

    doc.setFontSize(12);
    doc.text(`${data[0].discharge_m3hr}`, 10, 48);

    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(16);
    doc.text("Total Water Volume:", 10, 60);

    doc.setFontSize(12);
    doc.text(`${data[0].total_volume_water}`, 10, 68);

    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(16);
    doc.text("Tank Capacity:", 10, 80);

    doc.setFontSize(12);
    doc.text(`${data[0].tank_capacity_m3}`, 10, 88);

    // Save the PDF
    doc.save("Plumbing(Rainwater Harvested).pdf");
  };
  return (
    <div className="px-6">
      <div className="py-4">
